...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=64,
                        help='batch size')
    parser.add_argument('--n_hid', type=int, default=384)
...
def graph_collate(batch):
    ...
    batch_project_text_emb = default_collate([item[2] for item in batch])
    pos_person = default_collate([item[3] for item in batch])
    neg_person_list = default_collate([item[4] for item in batch])
    return project_id, batch_sub_g, batch_project_text_emb, pos_person, neg_person_list
...
def train(model, train_data_loader, valid_data_loader, test_data_loader, device, args):
    ...
    for epoch in np.arange(args.n_epoch) + 1:
        ...
        for step, batch_data in tqdm(enumerate(train_data_loader), total = n):
            project_id, sub_g, project_text_emb, pos_person, neg_person_list = batch_data
            ...
            pos_score = torch.sigmoid(torch.sum(project_text_emb * pos_person_emb, -1))
            neg_score = torch.sigmoid(torch.sum(project_text_emb * neg_person_emb, -1))
...
def eval(model, args, eval_data_loader):
    ...
    with torch.no_grad():
        for step, batch_data in tqdm(enumerate(eval_data_loader), total = n):
            ...
            score = torch.sigmoid(torch.sum(project_text_emb * eval_person_emb, -1))
